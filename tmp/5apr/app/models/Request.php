...
class Request extends Model implements ModelInterface {
 ...
 public function isThereAFriendRequest(){
    $logged_in_user = get_session_user();
    $query="SELECT u.id from {$this->table} r,users u WHERE  r.from_id=u.id and (r.to_id={$logged_in_user->id} or r.from_id={$logged_in_user->id})";
