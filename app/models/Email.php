...
class Email extends Model implements ModelInterface {
  ...
  public function searchMailBySubjectOrName($form){
    $logged_in_user = get_session_user();
    $query = "select u.fname,u.lname,u.id as uid,e.* from users u,emails e where ((e.to_id=u.id and e.from_id=$logged_in_user->id) or (e.from_id=u.id and e.to_id=$logged_in_user->id)) and(fname like '{$form}%' or subject like '%{$form}%')";
